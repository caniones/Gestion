inherited frmRepListaPrecioArt: TfrmRepListaPrecioArt
  Left = 344
  Top = 140
  Caption = 'frmRepListaPrecioArt'
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  inherited qrBase: TQuickRep
    DataSet = ibqMaster
    Functions.DATA = (
      '0'
      '0'
      #39#39)
    Page.Values = (
      100.000000000000000000
      2970.000000000000000000
      200.000000000000000000
      2100.000000000000000000
      200.000000000000000000
      100.000000000000000000
      0.000000000000000000)
    ReportTitle = 'Lista de Precios'
    inherited PageHeaderBand1: TQRBand
      Height = 77
      Size.Values = (
        203.729166666666700000
        1801.812500000000000000)
      inherited QRLabel3: TQRLabel
        Top = 40
        Size.Values = (
          44.979166666666670000
          994.833333333333400000
          105.833333333333300000
          378.354166666666700000)
        FontSize = 10
      end
      inherited QRSysData1: TQRSysData
        Top = 40
        Size.Values = (
          44.979166666666670000
          1397.000000000000000000
          105.833333333333300000
          179.916666666666700000)
        FontSize = 10
      end
      inherited qrlRenglon1: TQRLabel
        Size.Values = (
          79.375000000000000000
          21.166666666666670000
          2.645833333333333000
          455.083333333333300000)
        FontSize = 18
      end
      inherited qrlRenglon2: TQRLabel
        Size.Values = (
          44.979166666666670000
          21.166666666666670000
          76.729166666666680000
          259.291666666666700000)
        FontSize = 10
      end
      inherited qrlRenglon3: TQRLabel
        Size.Values = (
          44.979166666666670000
          21.166666666666670000
          124.354166666666700000
          232.833333333333400000)
        FontSize = 10
      end
      object QRLabel10: TQRLabel
        Left = 264
        Top = 1
        Width = 151
        Height = 20
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          52.916666666666660000
          698.500000000000000000
          2.645833333333333000
          399.520833333333400000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'LISTA DE PRECIOS'
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Arial'
        Font.Style = [fsBold, fsUnderline]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 12
      end
    end
    inherited PageFooterBand2: TQRBand
      Top = 289
      Height = 27
      Size.Values = (
        71.437500000000000000
        1801.812500000000000000)
      inherited QRLabel12: TQRLabel
        Size.Values = (
          44.979166666666670000
          1553.104166666667000000
          34.395833333333340000
          132.291666666666700000)
        FontSize = 10
      end
      inherited QRSysData4: TQRSysData
        Size.Values = (
          44.979166666666670000
          1714.500000000000000000
          34.395833333333340000
          121.708333333333300000)
        FontSize = 10
      end
      object QRLabel9: TQRLabel
        Left = 7
        Top = 7
        Width = 182
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          18.520833333333330000
          18.520833333333330000
          481.541666666666700000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = True
        Caption = 'El art'#237'culo se encuentra dado de baja.'
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsStrikeOut]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 8
      end
    end
    object SummaryBand1: TQRBand
      Left = 76
      Top = 241
      Width = 681
      Height = 48
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        127.000000000000000000
        1801.812500000000000000)
      BandType = rbSummary
      object QRLabel8: TQRLabel
        Left = 447
        Top = 13
        Width = 167
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1182.687500000000000000
          34.395833333333340000
          441.854166666666700000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'CANTIDAD DE ARTICULOS'
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 10
      end
      object QRExpr1: TQRExpr
        Left = 627
        Top = 13
        Width = 45
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1658.937500000000000000
          34.395833333333340000
          119.062500000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        Master = QRSubDetail1
        ResetAfterPrint = False
        Transparent = False
        WordWrap = True
        Expression = 'COUNT'
        FontSize = 10
      end
    end
    object QRSubDetail1: TQRSubDetail
      Left = 76
      Top = 228
      Width = 681
      Height = 13
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      BeforePrint = QRSubDetail1BeforePrint
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      ForceNewColumn = False
      ForceNewPage = False
      ParentFont = False
      Size.Values = (
        34.395833333333340000
        1801.812500000000000000)
      Master = qrBase
      DataSet = ibqDetalle
      PrintBefore = False
      PrintIfEmpty = True
      object QRDBText1: TQRDBText
        Left = 7
        Top = 0
        Width = 90
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          18.520833333333330000
          0.000000000000000000
          238.125000000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        DataSet = ibqDetalle
        DataField = 'CODIGO_INTERNO'
        Transparent = True
        WordWrap = True
        FontSize = 8
      end
      object QRDBText2: TQRDBText
        Left = 153
        Top = 0
        Width = 67
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          404.812500000000000000
          0.000000000000000000
          177.270833333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        DataSet = ibqDetalle
        DataField = 'DESCRIPCION'
        Transparent = True
        WordWrap = True
        FontSize = 8
      end
      object QRDBText3: TQRDBText
        Left = 501
        Top = 0
        Width = 86
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1325.562500000000000000
          0.000000000000000000
          227.541666666666700000)
        Alignment = taRightJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        DataSet = ibqDetalle
        DataField = 'PRECIO_PUBLICO'
        Transparent = True
        WordWrap = True
        FontSize = 8
      end
      object QRDBText4: TQRDBText
        Left = 626
        Top = 0
        Width = 35
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1656.291666666667000000
          0.000000000000000000
          92.604166666666680000)
        Alignment = taRightJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        DataSet = ibqDetalle
        DataField = 'FECHA'
        Transparent = True
        WordWrap = True
        FontSize = 8
      end
    end
    object QRGroup1: TQRGroup
      Left = 76
      Top = 201
      Width = 681
      Height = 27
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ForceNewColumn = False
      ForceNewPage = False
      ParentFont = False
      Size.Values = (
        71.437500000000000000
        1801.812500000000000000)
      Master = QRSubDetail1
      ReprintOnNewPage = True
      object QRLabel4: TQRLabel
        Left = 7
        Top = 7
        Width = 99
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          18.520833333333330000
          18.520833333333330000
          261.937500000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'CODIGO INTERNO'
        Color = clWhite
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel5: TQRLabel
        Left = 153
        Top = 7
        Width = 79
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          404.812500000000000000
          18.520833333333330000
          209.020833333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'DESCRIPCION'
        Color = clWhite
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel6: TQRLabel
        Left = 547
        Top = 7
        Width = 44
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1447.270833333333000000
          18.520833333333330000
          116.416666666666700000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'PRECIO'
        Color = clWhite
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel7: TQRLabel
        Left = 607
        Top = 7
        Width = 67
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1606.020833333333000000
          18.520833333333330000
          177.270833333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'FECHA ACT.'
        Color = clWhite
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
    end
    object DetailBand1: TQRBand
      Left = 76
      Top = 153
      Width = 681
      Height = 48
      Frame.Color = clBlack
      Frame.DrawTop = True
      Frame.DrawBottom = True
      Frame.DrawLeft = True
      Frame.DrawRight = True
      AlignToBottom = False
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold, fsItalic]
      ForceNewColumn = False
      ForceNewPage = False
      LinkBand = QRSubDetail1
      ParentFont = False
      Size.Values = (
        127.000000000000000000
        1801.812500000000000000)
      BandType = rbDetail
      object QRDBText5: TQRDBText
        Left = 7
        Top = 2
        Width = 47
        Height = 21
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          55.562500000000000000
          18.520833333333330000
          5.291666666666667000
          124.354166666666700000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        DataSet = ibqMaster
        DataField = 'TITULO'
        Transparent = False
        WordWrap = True
        FontSize = 10
      end
      object QRDBText6: TQRDBText
        Left = 7
        Top = 15
        Width = 40
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          18.520833333333330000
          39.687500000000000000
          105.833333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        DataSet = ibqMaster
        DataField = 'LINEA'
        Transparent = False
        WordWrap = True
        FontSize = 10
      end
      object QRDBText7: TQRDBText
        Left = 7
        Top = 28
        Width = 47
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          18.520833333333330000
          74.083333333333340000
          124.354166666666700000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        DataSet = ibqMaster
        DataField = 'RUBRO'
        Transparent = False
        WordWrap = True
        FontSize = 10
      end
    end
  end
  object ibqDetalle: TIBQuery
    Database = dmConexion.IBDatabase
    Transaction = dmConexion.ibTransac
    Active = True
    BufferChunks = 1000
    CachedUpdates = False
    DataSource = dsMaster
    SQL.Strings = (
      
        'SELECT A.CODIGO_INTERNO, A.DESCRIPCION, A.PRECIO_PUBLICO, A.FECH' +
        'A_ACTPRECIO AS FECHA, A.FECHA_BAJA'
      'FROM ARTICULOS A'
      'WHERE A.IDLINEA = :IDLINEA'
      'AND A.IDRUBRO = :IDRUBRO'
      'AND A.IDTITULO = :IDTITULO')
    Left = 115
    Top = 16
    ParamData = <
      item
        DataType = ftInteger
        Name = 'IDLINEA'
        ParamType = ptUnknown
        Size = 4
      end
      item
        DataType = ftInteger
        Name = 'IDRUBRO'
        ParamType = ptUnknown
        Size = 4
      end
      item
        DataType = ftInteger
        Name = 'IDTITULO'
        ParamType = ptUnknown
        Size = 4
      end>
    object ibqDetalleCODIGO_INTERNO: TIBStringField
      FieldName = 'CODIGO_INTERNO'
      Origin = 'ARTICULOS.CODIGO_INTERNO'
      Required = True
    end
    object ibqDetalleDESCRIPCION: TIBStringField
      FieldName = 'DESCRIPCION'
      Origin = 'ARTICULOS.DESCRIPCION'
      Required = True
      Size = 200
    end
    object ibqDetallePRECIO_PUBLICO: TIBBCDField
      FieldName = 'PRECIO_PUBLICO'
      Origin = 'ARTICULOS.PRECIO_PUBLICO'
      Required = True
      DisplayFormat = '$,0.##'
      EditFormat = '0.##'
      Precision = 18
      Size = 4
    end
    object ibqDetalleFECHA: TDateField
      FieldName = 'FECHA'
      Origin = 'ARTICULOS.FECHA_ACTPRECIO'
      Required = True
      DisplayFormat = 'dd/mm/yy'
    end
    object ibqDetalleFECHA_BAJA: TDateField
      FieldName = 'FECHA_BAJA'
      Origin = 'ARTICULOS.FECHA_BAJA'
    end
  end
  object ibqMaster: TIBQuery
    Database = dmConexion.IBDatabase
    Transaction = dmConexion.ibTransac
    Active = True
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT DISTINCT T.DESCRIPCION AS TITULO, L.DESCRIPCION AS LINEA,' +
        ' R.DESCRIPCION AS RUBRO, A.IDTITULO, A.IDLINEA, A.IDRUBRO'
      'FROM ARTICULOS A, LINEAS L, RUBROS R, TITULOS T'
      'WHERE A.IDLINEA = L.IDLINEA'
      'AND A.IDRUBRO = R.IDRUBRO'
      'AND A.IDTITULO = T.IDTITULO')
    Left = 48
    Top = 16
    object ibqMasterTITULO: TIBStringField
      FieldName = 'TITULO'
      Origin = 'TITULOS.DESCRIPCION'
      Required = True
      Size = 50
    end
    object ibqMasterLINEA: TIBStringField
      FieldName = 'LINEA'
      Origin = 'LINEAS.DESCRIPCION'
      Required = True
      Size = 50
    end
    object ibqMasterRUBRO: TIBStringField
      FieldName = 'RUBRO'
      Origin = 'RUBROS.DESCRIPCION'
      Required = True
      Size = 50
    end
    object ibqMasterIDTITULO: TIntegerField
      FieldName = 'IDTITULO'
      Origin = 'ARTICULOS.IDTITULO'
      Required = True
    end
    object ibqMasterIDLINEA: TIntegerField
      FieldName = 'IDLINEA'
      Origin = 'ARTICULOS.IDLINEA'
      Required = True
    end
    object ibqMasterIDRUBRO: TIntegerField
      FieldName = 'IDRUBRO'
      Origin = 'ARTICULOS.IDRUBRO'
      Required = True
    end
  end
  object dsMaster: TDataSource
    AutoEdit = False
    DataSet = ibqMaster
    Left = 80
    Top = 16
  end
end
